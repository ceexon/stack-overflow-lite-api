...
class TestUserSignUp(BaseTest):
    ...
    def test_signup_username_taken(self):
        response = self.client.post('api/v1/auth/signup', data=json.dumps(self.data7),content_type="application/json")
        result = json.loads(response.data.decode('utf-8'))
...
class TestUserLogin(BaseTest):
    def test_no_login_info(self):
        response = self.client.post('api/v1/auth/login', data=json.dumps(self.data1),content_type="application/json")
        result = json.loads(response.data.decode('utf-8'))
        self.assertEqual(result["message"],"No data found")
        self.assertEqual(response.status_code, 400)

    def test_missing_login_fields(self):
        response = self.client.post('api/v1/auth/login', data=json.dumps(self.data3),content_type="application/json")
        result = json.loads(response.data.decode('utf-8'))
        self.assertEqual(result["message"],"Missing either the username or password fields")
        self.assertEqual(response.status_code, 400)

    def test_empty_login_fieldvalues(self):
        response = self.client.post('api/v1/auth/login', data=json.dumps(self.data4),content_type="application/json")
        result = json.loads(response.data.decode('utf-8'))
        self.assertEqual(result["message"],"All fields must be filled")
        self.assertEqual(response.status_code, 400)

    def test_invalid_login_pass(self):
        response = self.client.post('api/v1/auth/login', data=json.dumps(self.data7), content_type="application/json")
        result = json.loads(response.data.decode('utf-8'))
        self.assertEqual(result["message"], "Invalid password")
        self.assertEqual(response.status_code, 401)
    
    def test_invalid_login_username(self):
        response = self.client.post('api/v1/auth/login', data=json.dumps(self.data8), content_type="application/json")
        result = json.loads(response.data.decode('utf-8'))
        self.assertEqual(result["message"], "username not found")
        self.assertEqual(response.status_code, 401)

    def test_login_successful(self):
        response = self.client.post('api/v1/auth/login', data=json.dumps(self.data2),content_type="application/json")
